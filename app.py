...
def detect_counter_trade(ind):
    """
    Spot short-term bounce opportunities that go AGAINST the primary downtrend.
    Conditions (all must be true):
      • Primary trend is bearish (EMA alignment or Supertrend)
      • RSI is deeply oversold (< 33) — sellers are exhausted
      • Price is at or below lower Bollinger Band (bb_pos < 0.18)
    Trade logic: quick bounce trade — enter now, target TP1 only, tight stop.
    Risk rating: HIGH (trading against the trend).
    """
    ...
    # Need bearish trend (EMA or Supertrend) + oversold + near lower band
    ...
    entry = price
    # Stop loss: just below recent support (or 1.5× ATR, whichever is tighter)
    sl = round(min(sup - atr * 0.3, price - atr * 1.5), 4)
    risk = entry - sl
    if risk <= 0:
        return {"counter_trade": False}

    # TP1 only — 1.5:1 R/R (quick exit near EMA20)
    ...
    summary = (
        ...
        f"EXIT at TP1 — do NOT hold this trade. HIGH RISK."
    )
...
def pre_screen(ind):
    """
    Quickly check if indicators suggest an actionable setup.
    Returns a dict so the alert system can decide whether to call Claude.
    No external API calls — purely maths on indicator values already computed.

    Scoring: each bullish/bearish condition adds +1/-1.
    |score| >= 2  → worth calling Claude.
    """
    rsi       = ind["rsi"]
    bb_pos    = ind["bb_pos"]
    ...
    # Volume surge boosts any signal
    vol_bonus = 1 if vol_ratio >= 1.5 else 0

    net_bull = bull_score + vol_bonus
    net_bear = bear_score + vol_bonus

    # Counter-trend bounce (already computed separately but check here too)
    counter_bounce = (
    ...
    if net_bull >= 4:
        ...
        hint, reason = None, "No clear setup — market is neutral"

    opportunity = hint is not None
    # Only worth the Claude API call for strong or counter signals
...
def screen():
    """
    Lightweight pre-screen endpoint — no Claude API call.
    Used by the alert watch system to check if conditions warrant a full analysis.
    Cost: free (yfinance only).
    """
    try:
        body       = request.json or {}
        ticker     = body.get("ticker", "").upper().strip()
