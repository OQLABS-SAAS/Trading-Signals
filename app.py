...
def send_email(subject, html_body, plain_body):
    ...
    to_email   = os.environ.get("ALERT_EMAIL", "").strip()
...
def build_alert_messages(signal, ticker, price, timeframe, analysis, counter):
    """Build SMS text + HTML/plain email for an alert."""
    ts = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")

    if signal == "COUNTER_BUY":
        emoji   = "⚡"
        subject = f"⚡ Counter-Trend Bounce — {ticker} @ ${price} [{timeframe.upper()}]"
        sms     = (
        ...
        plain = (
            f"Counter-Trend Bounce Detected — {ticker} ({timeframe.upper()})\n"
            f"{'='*50}\n"
            f"Price:      ${price}\n"
            f"Entry:      ${counter.get('counter_entry')}\n"
            f"Stop Loss:  ${counter.get('counter_sl')}\n"
            f"TP1 (exit): ${counter.get('counter_tp1')}  (R/R {counter.get('counter_rr1')}:1)\n\n"
            f"Summary: {counter.get('counter_summary', '')}\n\n"
            f"⚠ HIGH RISK trade against the downtrend. Exit at TP1. Reduce position size.\n\n"
            f"Generated: {ts}\nFor informational purposes only. Not financial advice."
        )
        html = f"""
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#0e1520;color:#e2e8f0;border-radius:12px;overflow:hidden">
  <div style="background:linear-gradient(135deg,rgba(251,146,60,0.2),rgba(234,88,12,0.1));padding:24px;border-bottom:1px solid rgba(251,146,60,0.3)">
    <div style="font-size:28px;font-weight:800;color:#fb923c">⚡ COUNTER-TREND BOUNCE</div>
    <div style="font-size:20px;font-weight:600;margin-top:6px">{ticker} — {timeframe.upper()}</div>
    <div style="font-size:24px;font-weight:700;color:#fb923c;margin-top:4px">${price}</div>
    <div style="background:rgba(255,71,87,0.2);color:#ff4757;padding:4px 12px;border-radius:20px;display:inline-block;font-size:12px;font-weight:700;margin-top:8px">⚠ HIGH RISK</div>
  </div>
  <div style="padding:24px">
    <p style="color:#94a3b8;font-size:14px;line-height:1.6">{counter.get('counter_summary','')}</p>
    <table style="width:100%;border-collapse:collapse;margin-top:16px">
      <tr><td style="padding:10px;background:#141d2e;border-radius:6px;color:#64748b;font-size:12px;text-transform:uppercase">Entry</td><td style="padding:10px;font-weight:700;color:#60a5fa;font-size:16px">${counter.get('counter_entry')}</td></tr>
      <tr><td style="padding:10px;color:#64748b;font-size:12px;text-transform:uppercase">Stop Loss</td><td style="padding:10px;font-weight:700;color:#ff4757;font-size:16px">${counter.get('counter_sl')}</td></tr>
      <tr><td style="padding:10px;background:#141d2e;border-radius:6px;color:#64748b;font-size:12px;text-transform:uppercase">TP1 — EXIT HERE</td><td style="padding:10px;font-weight:700;color:#4ade80;font-size:16px">${counter.get('counter_tp1')} &nbsp;<span style="font-size:12px;color:#64748b">R/R {counter.get('counter_rr1')}:1</span></td></tr>
    </table>
    <div style="background:rgba(251,146,60,0.08);border:1px solid rgba(251,146,60,0.25);border-radius:8px;padding:12px;margin-top:16px;font-size:13px;color:#fb923c">
      Exit at TP1 only. Do not hold beyond first target. Reduce position size to 50% of normal.
    </div>
  </div>
  <div style="padding:12px 24px;border-top:1px solid #1e2d45;font-size:11px;color:#64748b">{ts} · SignalAI · For informational purposes only. Not financial advice.</div>
</div>"""

    else:
        is_buy  = signal == "BUY"
        color   = "#00d4aa" if is_buy else "#ff4757"
        emoji   = "🟢" if is_buy else "🔴"
        subject = f"{emoji} {signal} Signal — {ticker} @ ${price} [{timeframe.upper()}]"
        sms     = (
        ...
        plain = (
            f"{signal} Signal — {ticker} ({timeframe.upper()})\n"
            f"{'='*50}\n"
            f"Price:      ${price}\n"
        ...
        html = f"""
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#0e1520;color:#e2e8f0;border-radius:12px;overflow:hidden">
  <div style="background:linear-gradient(135deg,rgba({'0,212,170' if is_buy else '255,71,87'},0.15),rgba({'0,212,170' if is_buy else '255,71,87'},0.05));padding:24px;border-bottom:1px solid rgba({'0,212,170' if is_buy else '255,71,87'},0.3)">
    <div style="font-size:28px;font-weight:800;color:{color}">{emoji} {signal} SIGNAL</div>
    <div style="font-size:20px;font-weight:600;margin-top:6px">{ticker} — {timeframe.upper()}</div>
    <div style="font-size:24px;font-weight:700;color:{color};margin-top:4px">${price}</div>
    <div style="background:rgba({'0,212,170' if is_buy else '255,71,87'},0.15);color:{color};padding:4px 12px;border-radius:20px;display:inline-block;font-size:12px;font-weight:700;margin-top:8px">{analysis.get('confidence','—')} CONFIDENCE</div>
  </div>
  <div style="padding:24px">
    <p style="color:#94a3b8;font-size:14px;line-height:1.6">{analysis.get('summary','')}</p>
    <table style="width:100%;border-collapse:collapse;margin-top:16px">
      <tr><td style="padding:10px;background:#141d2e;border-radius:6px;color:#64748b;font-size:12px;text-transform:uppercase">Entry</td><td style="padding:10px;font-weight:700;color:#60a5fa;font-size:16px">${analysis.get('entry')}</td></tr>
      <tr><td style="padding:10px;color:#64748b;font-size:12px;text-transform:uppercase">Stop Loss</td><td style="padding:10px;font-weight:700;color:#ff4757;font-size:16px">${analysis.get('stop_loss')}</td></tr>
      <tr><td style="padding:10px;background:#141d2e;border-radius:6px;color:#64748b;font-size:12px;text-transform:uppercase">TP1 🎯</td><td style="padding:10px;font-weight:700;color:#4ade80;font-size:16px">${analysis.get('tp1')} &nbsp;<span style="font-size:12px;color:#64748b">R/R {analysis.get('rr1')}:1</span></td></tr>
      <tr><td style="padding:10px;color:#64748b;font-size:12px;text-transform:uppercase">TP2 🎯🎯</td><td style="padding:10px;font-weight:700;color:{color};font-size:16px">${analysis.get('tp2')} &nbsp;<span style="font-size:12px;color:#64748b">R/R {analysis.get('rr2')}:1</span></td></tr>
      <tr><td style="padding:10px;background:#141d2e;border-radius:6px;color:#64748b;font-size:12px;text-transform:uppercase">TP3 🚀</td><td style="padding:10px;font-weight:700;color:{color};font-size:16px">${analysis.get('tp3')} &nbsp;<span style="font-size:12px;color:#64748b">R/R {analysis.get('rr3')}:1</span></td></tr>
    </table>
  </div>
  <div style="padding:12px 24px;border-top:1px solid #1e2d45;font-size:11px;color:#64748b">{ts} · SignalAI · For informational purposes only. Not financial advice.</div>
</div>"""
...
def fire_alert(signal, ticker, price, timeframe, analysis, counter):
    subject, sms_text, html_body, plain_body = build_alert_messages(
        signal, ticker, price, timeframe, analysis, counter
    )
    send_sms(sms_text)
