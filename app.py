...
def safe_download(ticker, **kwargs):
    """Wrapper around yf.download that always returns a flat-column DataFrame.

    yfinance ≥ 0.2 returns a MultiIndex even for a single ticker:
      columns = [("Close","ETH-USD"), ("High","ETH-USD"), ...]
    Calling df["Close"] on that raises KeyError.  We drop the ticker level
    so callers can always use df["Close"], df["High"], etc. without changes.
    """
    df = yf.download(ticker, **kwargs)
    if isinstance(df.columns, pd.MultiIndex):
        # Level 0 = OHLCV name, Level 1 = ticker symbol — keep only level 0
...
def _tv_symbol_market(ticker, asset_type):
    ...
    if asset_type == "forex":
        pair = ticker.replace("=X","").replace("/","")
        return f"FX_IDC:{pair}", "forex"
...
def fetch_tv_data(raw_ticker, asset_type):
    """Fetch TradingView indicator snapshot for a single ticker.
    Returns an enrichment dict or None if the call fails."""
    try:
        tv_sym, market = _tv_symbol_market(raw_ticker, asset_type)
        url = _TV_SCANNER.get(asset_type, _TV_SCANNER["stock"])

        # Core columns — available for all asset types
        columns = [
            "close", "RSI", "MACD.macd", "MACD.signal",
            "EMA20", "EMA50", "EMA200",
            "BB.upper", "BB.lower",
        ...
        if asset_type == "stock":
            columns += ["P/E", "market_cap_basic",
                        "earnings_per_share_basic_ttm", "analyst_count",
                        "Perf.Y"]

        resp = requests.post(
            url,
            json={"symbols": {"tickers": [tv_sym]}, "columns": columns},
        ...
        rsi       = g("RSI")
        macd_l    = g("MACD.macd")
        macd_s    = g("MACD.signal")
        macd_hist = round(macd_l - macd_s, 6) if (macd_l is not None and macd_s is not None) else None
        ema20     = g("EMA20")
        ema50     = g("EMA50")
        ema200    = g("EMA200")
        bb_upper  = g("BB.upper")
        ...
        result = {
            "tv_symbol":      tv_sym,
            "tv_price":       round(close, 6) if close else None,
...
def analyze():
    try:
        ...
        # ── TradingView enrichment (best-effort, never blocks analysis) ──
        tv = fetch_tv_data(ticker, asset_type)

        analysis = get_analysis(ticker, asset_type, ind, timeframe, tv=tv)
        counter  = detect_counter_trade(ind)

        df_daily = safe_download(ticker, period="1y", interval="1d",
        ...
        return jsonify({
            ...
            **analysis,
            ...
            "mtf":         mtf,
            **counter,
            # TradingView cross-verification data (None fields omitted by caller)
            "tv": tv,
        })
        ...
        msg    = client.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    ...
                    "Be direct, trader-focused. No fluff. No bullet points."
                )
            }]
        )
        return jsonify({"brief": msg.content[0].text, "date": today})
    except Exception as e:
        return jsonify({
            "brief": "Daily brief temporarily unavailable. Check API key configuration.",
            "error": str(e)
        })
