...
|---|---|---|---|---|---|---|---|
| **SIG-1** | ✅ | `app.py` | 3685 | Remove `if not tv_signal_used` from Gate 1 (HTF) and Gate 2 (footprint). Both gates now run regardless of signal source. | ✅ DONE — gates run on ALL signals. No tv_signal_used guard exists. |
| **SIG-2** | ✅ | `app.py` | 4077 | confidence_label now derived from final `confidence` string (HIGH→CONFIRMED, MEDIUM→LIKELY, LOW→HYPOTHESIS). No TV gating. Three-way consistency with confidence_pct. | ✅ DONE — label matches confidence level. |
| **SIG-3** | ✅ | `app.py` | 3957 | RANGING + BUY/SELL now downgrades confidence one level + shows plain-English warning. Updated from block-to-HOLD to warn-and-downgrade — better for beginners who need actionable signals even in choppy conditions. | ✅ DONE — warns + downgrades, does not block. |
| **SIG-4** | ✅ | `app.py` | 3717 | RSI divergence now silently drops confidence one level. No text injected — beginner sees adjusted verdict without internal math debate. Commit: 6e95ab5. | ✅ DONE — silent penalty, no contradiction text. |
| **SIG-5** | ✅ | `app.py` | 3339 | EMA vote weight reduced to +1 per direction. STRONG preserved in trend_assessment text only. MIN_VOTES gate prevents single-indicator fires. | ✅ DONE — one indicator = one vote. |
| **SIG-6** | ✅ | `app.py` | 906 | Swing pivot S/R detection implemented. Pivot = bar with higher high/lower low than 3 bars each side. Nearest pivot above price = resistance, below = support. Fallback to 10-bar rolling if < 2 pivots found. | ✅ DONE — structural levels. |
| **SIG-7** | ✅ | `app.py` | 1097 | `_tv_vol_ratio()` fetches real volume from yfinance with 5-min in-memory cache. Forex bypassed (no centralized volume). `calculate_indicators` always preferred over `build_ind_from_tv` when yfinance is available (Bug N fix). | ✅ DONE — real volume on all paths. |
